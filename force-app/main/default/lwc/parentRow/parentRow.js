...
export default class ParentRow extends LightningElement {
    ...
    handleInit(){
        ...
        if(this.editModeEnabled && this.parentRowData['Id']){
            this._parentRecordId=this.parentRowData['Id'];
        }
    }
    ...
    handleEdit(){
        ...
        this.formRecordId=this._parentRecordId;
        this.formHeader='Edit Record';
    }
    handleDelete(){
        deleteRecord(this._parentRecordId)
            .then(()=>{
    ...
    handleFormSuccess(event){
        ...
        //set delay so that data can be refreshed from apex
        setTimeout(()=>{this.notifyDataChange(this);}, 1000);
    }
